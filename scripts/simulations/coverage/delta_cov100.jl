...
correct_delta = fill(NaN, 3, sims)
under_delta = fill(NaN, 3, sims)
over_delta = fill(NaN, 3, sims)
...
@showprogress Threads.@threads for i = 1:sims
    ...
    correct_reg = comovement_reg(cen_data, dimvals, true_ranks, iters=50)
    over_reg = comovement_reg(cen_data, dimvals, over_rank, iters=50)
    under_reg = comovement_reg(cen_data, dimvals, under_rank, iters=50)
