...
smallaic83 = fill(NaN, 2, sims)
smallbic83 = fill(NaN, 2, sims)
medaic83 = fill(NaN, 2, sims)
medbic83 = fill(NaN, 2, sims)
...
@showprogress Threads.@threads for s = 1:sims
    ...
    smallaic83[:, s] .= smallicest.aic_sel[1:2]
    smallbic83[:, s] .= smallicest.bic_sel[1:2]

    medicest = rank_selection(medmar.data, dimvals; iters=200)
    medaic83[:, s] .= medicest.aic_sel[1:2]
    medbic83[:, s] .= medicest.bic_sel[1:2]
end

save(datadir("eightbythree/83_results.jld2"), Dict(
    "smallaic" => smallaic83,
    "smallbic" => smallbic83,
    "medaic" => medaic83,
    "medbic" => medbic83
...
medaicstats = sim_stats(medaic83, ranks, sims)
medbicstats = sim_stats(medbic83, ranks, sims)
smallaicstats = sim_stats(smallaic83, ranks, sims)
smallbicstats = sim_stats(smallbic83, ranks, sims)
