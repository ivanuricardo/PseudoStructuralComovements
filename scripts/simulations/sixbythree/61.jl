...
smallaic61 = fill(NaN, 2, sims)
smallbic61 = fill(NaN, 2, sims)
medaic61 = fill(NaN, 2, sims)
medbic61 = fill(NaN, 2, sims)

smallbic61_bench = fill(NaN, 2, sims)
medbic61_bench = fill(NaN, 2, sims)
...
@showprogress Threads.@threads for s = 1:sims
    ...
    smallaic61[:, s] .= smallicest.aic_sel[1:2]
    smallbic61[:, s] .= smallicest.bic_sel[1:2]

    medicest = rank_selection(medmar.data, dimvals; iters=200)
    medaic61[:, s] .= medicest.aic_sel[1:2]
    medbic61[:, s] .= medicest.bic_sel[1:2]
    ...
    smallbic61_bench[:, s] .= rcopy(small_bench)
    medbic61_bench[:, s] .= rcopy(med_bench)
end

save(datadir("sixbythree/61_results.jld2"), Dict(
    "smallaic" => smallaic61,
    "smallbic" => smallbic61,
    "medaic" => medaic61,
    "medbic" => medbic61,
    "smallbic_bench" => smallbic61_bench,
    "medbic_bench" => medbic61_bench,
))

medaicstats = sim_stats(medaic61, ranks, sims)
medbicstats = sim_stats(medbic61, ranks, sims)
smallaicstats = sim_stats(smallaic61, ranks, sims)
smallbicstats = sim_stats(smallbic61, ranks, sims)
smallbicstats_bench = sim_stats(smallbic61_bench, ranks, sims)
medbicstats_bench = sim_stats(medbic61_bench, ranks, sims)
