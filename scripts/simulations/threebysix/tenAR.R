...
tenAR.sim <- function(t, dim, R, P, rho, cov, A = NULL, Sig = NULL) {
    ...
    for (l in c(1:P)) {
        if (R[l] == 0) {
            ...
            phi[[l]] <- Reduce("+", lapply(1:R[l], function(j) {
                rTensor::kronecker_list(rev(A[[l]][[j]]))
            }))
        }
    }
...
taxi.sim.AR <- function(t, print.tar.coef = FALSE, print.sig = FALSE, seed = 123) {
    ...
    Tar.coef[[1]][[1]][[3]] <-
        array(c(
            ...
            0.1707, 0.3203, 0.2702, 0.1234, 0.2110, 0.2197, 0.4511
        ), c(7, 7))
    Tar.coef[[1]][[2]][[1]] <-
        array(c(
            0.4287, -0.0358, 0.0367, -0.0044, 0.0043,
            -0.0723, 0.3955, 0.0167, -0.0178, -0.0271,
            -0.0556, 0.0213, 0.2698, -0.1532, 0.0222,
            0.1399, 0.1557, 0.0894, 0.6041, 0.12063,
            -0.0425, -0.0380, 0.0340, 0.1001, 0.3234
        ), c(5, 5))
    Tar.coef[[1]][[2]][[2]] <-
        array(c(
            -0.3992, -0.0254, 0.0234, 0.0077, 0.0580,
            -0.1088, -0.3678, -0.1012, 0.0192, -0.0521,
            -0.1378, -0.1245, -0.4167, -0.0181, -0.1964,
            -0.1222, -0.0307, -0.0721, -0.3552, -0.1618,
            0.0074, -0.0325, -0.0861, -0.0201, -0.4961
        ), c(5, 5))
    Tar.coef[[1]][[2]][[3]] <-
        array(c(
            -1.0060, -0.1431, -0.0633, -0.2255, -0.0677, -0.0829, 0.0056,
            ...
            -0.0455, -0.2491, -0.3330, -0.3430, -0.3031, -0.3690, -0.6041
        ), c(7, 7))
    Tar.sig[[1]] <-
        array(c(
            0.7754, 0.1420, 0.1163, 0.0663, 0.1333,
            0.1420, 0.5846, 0.0773, 0.0522, 0.0844,
            0.1163, 0.0773, 0.4227, 0.0455, 0.0829,
            0.0663, 0.0522, 0.0455, 0.7122, 0.0454,
            0.1333, 0.0844, 0.0829, 0.0454, 0.5275
        ), c(5, 5))
    Tar.sig[[2]] <-
        array(c(
            0.8059, 0.1585, 0.1290, 0.0492, 0.1474,
            0.1585, 0.4694, 0.0801, 0.0240, 0.0840,
            0.1290, 0.0801, 0.4033, 0.0299, 0.0935,
            0.0492, 0.0240, 0.0299, 0.3690, 0.0320,
            0.1474, 0.0840, 0.0935, 0.0320, 0.5459
        ), c(5, 5))
    Tar.sig[[3]] <-
        array(c(
            156.7240, 38.4326, 26.3676, 23.1721, 18.9742, 16.0869, 16.2405,
            ...
            16.2405, 17.6571, 20.1618, 25.9977, 30.9949, 36.1325, 157.0083
        ), c(7, 7))
    set.seed(seed)
...
taxi.sim.FM <- function(t, print.tar.coef = FALSE, print.loading = FALSE, seed = 216) {
    ...
    Tar.coef[[1]][[1]][[1]] <- array(c(
        -0.4163, 0.0603, -0.0199, 0.0598,
        -0.1268, -0.6219, -0.0551, -0.0251,
        -0.0127, -0.0001, -0.4572, -0.0376,
        0.0609, 0.0252, 0.0629, -0.4402
    ), c(4, 4))
    Tar.coef[[1]][[1]][[2]] <- array(c(
        -0.5453, -0.0369, 0.0001, -0.1130,
        -0.0373, -0.3590, -0.0214, 0.0495,
        0.0143, 0.0460, -0.5629, 0.1233,
        -0.0004, -0.0562, -0.0165, -0.4665
    ), c(4, 4))
    Tar.coef[[1]][[1]][[3]] <- array(c(
        2.4676, -0.1332, -0.8460,
        0.0790, 2.7971, 1.0344,
        0.0161, 0.1530, 2.2103
    ), c(3, 3))

    set.seed(seed)
    Ft.sim <- tenAR.sim(t, c(4, 4, 3), 1, 1, 0.75, cov = "iid", A = Tar.coef)
    TenFM.loading <- list()
    TenFM.loading[[1]] <- array(c(
        -0.0174, 0.5156, 0.7721, -0.0091,
        ...
        -0.0499, 0.8853, 0.2375, 0.2719
    ), c(12, 4))
    TenFM.loading[[2]] <- array(c(
        0.0702, 0.1259, 0.0871, 0.0326,
        ...
        0.3524, -0.1841, 0.0998, 0.1658
    ), c(12, 4))
    TenFM.loading[[3]] <- array(c(
        0.0154, -0.0069, -0.0202, -0.0274, 0.012, 0.1211,
        ...
        0.2854, 0.3422, 0.3934, 0.391, 0.3234, 0.2422
    ), c(24, 3))
    if (print.tar.coef == TRUE) {
    ...
    Xt.sim <- tensor(
        tensor(
            tensor(Ft.sim, TenFM.loading[[1]], 2, 2),
            TenFM.loading[[2]], 2, 2
        ),
        TenFM.loading[[3]], 2, 2
    )
...
MAR1.RRLS.SE <- function(A1, A2, k1, k2, Sigma.e, RU1 = diag(k1), RV1 = diag(k1), RU2 = diag(k2), RV2 = diag(k2), mpower = 100) {
    ...
    if (k1 < d1) {
        R1u <- cbind(kronecker(diag(k1), U1), kronecker(diag(k1), U1c)) %*% rbind(
            solve(kronecker(D1^2, diag(k1)) -
        ...
        R1v <- cbind(kronecker(diag(k1), V1), kronecker(diag(k1), V1c)) %*% rbind(
            solve(kronecker(D1^2, diag(k1)) -
    ...
    if (k2 < d2) {
        R2u <- cbind(kronecker(diag(k2), U2), kronecker(diag(k2), U2c)) %*% rbind(
            solve(kronecker(D2^2, diag(k2)) -
        ...
        R2v <- cbind(kronecker(diag(k2), V2), kronecker(diag(k2), V2c)) %*% rbind(
            solve(kronecker(D2^2, diag(k2)) -
    ...
    return(list(
        "Sigma" = Sigma.LS, "Theta1.u" = Theta1.LS.u, "Theta1.v" = Theta1.LS.v,
...
MAR1.RRCC.SE <- function(A1, A2, k1, k2, Sigma1, Sigma2, RU1 = diag(k1), RV1 = diag(k1), RU2 = diag(k2), RV2 = diag(k2), mpower = 100) {
    ...
    return(list(
        "Sigma" = Sigma.CC, "Theta1.u" = Theta1.CC.u, "Theta1.v" = Theta1.CC.v,
...
MAR.SE <- function(xx, B, A, Sigma) {
    ...
    BXI <- apply(BX, 1, function(x) {
        kronecker(t(x), diag(p))
    })
    AXI <- apply(AX, 1, function(x) {
        kronecker(diag(q), t(x))
    })
...
tenAR.A <- function(dim, R, P, rho) {
    K <- length(dim)
    A <- lapply(1:P, function(p) {
        lapply(1:max(1, R[p]), function(j) {
...
tenAR.LS <- function(xx, R, P, init.A = NULL, niter = 150, tol = 1e-5, print.true = FALSE) {
    ...
    if (length(P) == 2) {
        ...
        rolling <- FALSE
    } # len(P) > 1 implies rolling in func 'tenAR.predict'
    ...
    if (K == 2 | K > 3) { # for initialization
        if (is.null(init.A)) {
            ...
            for (p in c(1:P)) {
                if (is.na(R[p])) stop("p != length(R)")
                if (R[p] == 0) next
                A.old[[p]] <- lapply(1:R[p], function(j) {
                    lapply(1:K, function(i) {
                        0.5 * diag(dim[i])
                    })
                })
        ...
        if (is.null(init.A)) {
            A.old <- tenAR.PROJ(xx@data, R, P)$A
        } else {
            A.old <- init.A
        }
    } else {
        stop("dimension K of time series must at least be 2")
    }
            ...
            for (r in c(1:R[p])) {
                for (k in c(K:1)) { # update last matrix first
                    ...
                    for (l in c(1:P)) {
                        ...
                        L1 <- L1 + Reduce("+", lapply(rMode, function(n) {
                            rTensor::ttl(subxx[[l + 1]], A.new[[l]][[n]], ttlMode)
                        }))
                    }
                    temp2 <- subxx[[1]] - L1
...
tenAR.MLE <- function(xx, R, P, init.A = NULL, init.sig = NULL, niter = 150, tol = 1e-5, print.true = FALSE) {
    ...
    Sig.new <- Sig.old
    Sig.new.inv <- lapply(1:K, function(k) {
        solve(Sig.new[[k]])
    })
    A.new <- A.old
    Tol <- tol * sqrt(sum(dim^2)) * sum(R)
    ...
    while (iiter <= niter & dis >= Tol) {
        for (p in c(1:P)) {
            ...
            for (r in c(1:R[p])) {
                ...
                for (k in c(K:1)) {
                    sphi <- lapply(1:K, function(k) {
                        Sig.new.inv[[k]] %*% (A.new[[p]][[r]][[k]])
                    })
...
MAR1.CC <- function(xx, k1, k2, A1.init = NULL, A2.init = NULL, Sigl.init = NULL, Sigr.init = NULL, niter = 200, tol = 1e-4, print.true = FALSE) {
    ...
    return(list(A1 = LL, A2 = RR, loading = loading, res = res, Sig1 = Sigl, Sig2 = Sigr, Sig = Sig, niter = iiter - 1, data = xx))
}
    ...
    for (i in c(1:p)) {
        for (j in c(1:r[i])) {
            for (k in c(1:K)) {
                ...
                AXI <- apply(AXfold, 3, function(x) {
                    kronecker(x, diag(dim[k])) %*% Q[[k]]
                })
    ...
    for (tt in ttt) {
        ...
        for (l in c(1:P)) {
            ...
            L1 <- L1 + Reduce("+", lapply(c(1:R[l]), function(n) {
                rTensor::ttl(abind::asub(xx, tt - l, 1, drop = FALSE), A[[l]][[n]], (c(1:K) + 1))
            }))
        }
...
predRolling <- function(A, xx, n.ahead, method, n0, print.true = TRUE) {
    ...
    for (tt in ttt) {
        tti <- tt - ttt[1] + 1
        if (print.true == TRUE) {
            cat(paste("rolling forcast t =", tti))
        }
        if (method == "RRLSE") {
        ...
        for (l in c(1:P)) {
            if (method == "VAR") {
                ...
                if (R[l] == 0) next
                L1 <- L1 + Reduce("+", lapply(c(1:R[l]), function(n) {
                    rTensor::ttl(abind::asub(rTensor::as.tensor(xx), tt - l + 1, 1, drop = FALSE), A[[l]][[n]], (c(1:K) + 1))
                }))@data
            }
        }
        ...
        if (tti == 1) {
            ...
            xx.pred <- abind(xx.pred, L1, along = 1)
        }
    }
    return(xx.pred)
}
