...
density_results = load(datadir("coverage/severe_case.jld2"))
...
vline!([delta_true[2]])
vline!([mean(under_delta[1, :])])
savefig(h1, "../density_plots/delta_density100_1.png")

h2 = density(correct_delta[2, :], legend=false)
density!(under_delta[2, :])
density!(over_delta[2, :])
vline!([delta_true[3]])
vline!([mean(under_delta[2, :])])
savefig(h2, "../density_plots/delta_density100_2.png")
