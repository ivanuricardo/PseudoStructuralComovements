...
# Starts at 1991-Q1 until 2019-Q4, for 115 observations
matdata = fill(NaN, 3, 4, 115)
for (i, country) in enumerate(countries)
    matdata[1, i, :] = cleancountry(country, gdp, logtransform=true, diffval=1)
end
for (i, country) in enumerate(countries)
    matdata[2, i, :] = cleancountry(country, production, logtransform=true, diffval=1)
end
for (i, country) in enumerate(countries)
    matdata[3, i, :] = cleancountry(country, interestrate, diffval=1)
end
