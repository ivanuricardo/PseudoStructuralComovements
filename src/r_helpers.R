...
fro.order <- function(A) {
    ...
    A.norm <- c()
    for (j in c(1:R)) {
        A.norm[j] <- Reduce("*", lapply(c(1:K), function(k) {
            norm(A[[j]][[k]], "f")
        }))
    }
...
ten.res <- function(subxx, A, P, R, K, ttlMode) {
    ...
    for (l in c(1:P)) {
        if (R[l] == 0) next
        L1 <- L1 + Reduce("+", lapply(c(1:R[l]), function(n) {
            rTensor::ttl(subxx[[l + 1]], A[[l]][[n]], ttlMode)
        }))
    }
    res <- subxx[[1]] - L1
    return(res)
...
M.eigen <- function(A, R, P, dim) {
    ...
    for (i in c(1:P)) {
        ...
        if (R[i] == 0) {
            ...
            phi[[i]] <- Reduce("+", lapply(1:R[i], function(j) {
                rTensor::kronecker_list(rev(A[[i]][[j]]))
            }))
        }
        if (i == 1) {
            M <- phi[[1]]
...
EBIC <- function(residuals, obs, d1, d2, r1, r2) {
    RSS <- sum(residuals^2)
    mse <- RSS / (obs * d1 * d2)
    penalty <- log(obs * d2) * r1 * (2 * d1 - r1) + log(obs * d1) * r2 * (2 * d2 - r2)
    penalty <- penalty / (obs * d1 * d2)
    return(log(mse) + penalty)
}

r_rank_selection <- function(data, d1, d2) {
    rank_grid <- expand.grid(r1 = 1:d1, r2 = 1:d2)
    obs <- dim(data)[1]
    ic_results <- matrix(NaN, nrow = d1 * d2, ncol = 3)
    for (i in 1:(d1 * d2)) {
        selected_rank <- rank_grid[i, ]
        r1 <- selected_rank[[1]]
        r2 <- selected_rank[[2]]
        est <- matAR.RR.est(data, method = "RRMLE", k1 = r1, k2 = r2)
        ebic <- EBIC(est$res, obs, d1, d2, r1, r2)
        ic_results[i, 1] <- ebic[1]
        ic_results[i, 2] <- r1
        ic_results[i, 3] <- r2
    }
